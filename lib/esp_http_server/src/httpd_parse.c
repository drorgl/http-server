...
static esp_err_t verify_url (http_parser *parser)
{
    ...
    if (r->method < 0) {
        LOGW(TAG, LOG_FMT("HTTP method not supported (%d)"), r->method);
        parser_data->error = HTTPD_501_METHOD_NOT_IMPLEMENTED;
        return ESP_FAIL;
    }

    if (sizeof(r->uri) < (length + 1)) {
        LOGW(TAG, LOG_FMT("URI length (%"NEWLIB_NANO_COMPAT_FORMAT") greater than supported (%"NEWLIB_NANO_COMPAT_FORMAT")"),
    ...
    if (!((parser->http_major == 1) && ((parser->http_minor == 0) || (parser->http_minor == 1)))) {
        LOGW(TAG, LOG_FMT("unsupported HTTP version = %d.%d"),
                        ...
                        const char *at, size_t length)
{
    ...
    if (parser_data->status == PARSING_IDLE) {
        ...
        LOGE(TAG, LOG_FMT("unexpected state transition"));
...
static esp_err_t pause_parsing(http_parser *parser, const char* at)
{
    ...
    if (unparsed < 0) {
        LOGE(TAG, LOG_FMT("parsing beyond valid data = %d"), (int)(-unparsed));
    ...
    if (unparsed && (unparsed != httpd_unrecv(r, at, unparsed))) {
        LOGE(TAG, LOG_FMT("data too large for un-recv = %d"), (int)unparsed);
        return ESP_FAIL;
    }
...
static size_t continue_parsing(http_parser *parser, size_t length)
{
    ...
    LOGD(TAG, LOG_FMT("skip pre-parsed data of size = %"NEWLIB_NANO_COMPAT_FORMAT), NEWLIB_NANO_COMPAT_CAST(length));

    http_parser_pause(parser, 0);
    data->paused = false;
    LOGD(TAG, LOG_FMT("un-paused"));
    return length;
}
...
static esp_err_t cb_header_field(http_parser *parser, const char *at, size_t length)
{
    ...
    if (parser_data->status == PARSING_URL) {
        ...
        LOGD(TAG, LOG_FMT("headers begin"));
    ...
    LOGD(TAG, LOG_FMT("processing field = %.*s"), (int)length, at);
...
static esp_err_t cb_header_value(http_parser *parser, const char *at, size_t length)
{
    ...
    LOGD(TAG, LOG_FMT("processing value = %.*s"), (int)length, at);
...
static esp_err_t cb_headers_complete(http_parser *parser)
{
    ...
    if (parser_data->status == PARSING_URL) {
        ...
        if (remaining_length < 2) {
            LOGE(TAG, LOG_FMT("invalid length of data remaining to be parsed"));
            parser_data->error = HTTPD_500_INTERNAL_SERVER_ERROR;
            parser_data->status = PARSING_FAILED;
            return ESP_FAIL;
        }
        ...
        if (remaining_terminators) {
            LOGE(TAG, LOG_FMT("incomplete termination of headers"));
            parser_data->error = HTTPD_400_BAD_REQUEST;
            parser_data->status = PARSING_FAILED;
            return ESP_FAIL;
        }
    ...
    if (parser->upgrade) {
        ...
        LOGD(TAG, LOG_FMT("Got an upgrade request"));
        ...
        if (httpd_req_get_hdr_value_str(r, "Upgrade", ws_upgrade_hdr_val, sizeof(ws_upgrade_hdr_val)) != ESP_OK) {
            LOGW(TAG, LOG_FMT("Upgrade header does not match the length of \"websocket\""));
            parser_data->error = HTTPD_400_BAD_REQUEST;
            parser_data->status = PARSING_FAILED;
            return ESP_FAIL;
        }
        ...
        if (strcasecmp("websocket", ws_upgrade_hdr_val) != 0) {
            LOGW(TAG, LOG_FMT("Upgrade header found but it's %s"), ws_upgrade_hdr_val);
            parser_data->error = HTTPD_400_BAD_REQUEST;
            parser_data->status = PARSING_FAILED;
            return ESP_FAIL;
        }
...
#else
        LOGD(TAG, LOG_FMT("WS functions has been disabled, Upgrade request is not supported."));
        parser_data->error = HTTPD_400_BAD_REQUEST;
        parser_data->status = PARSING_FAILED;
        return ESP_FAIL;
#endif
...
static esp_err_t cb_no_body(http_parser *parser)
{
    ...
    parser_data->last.length = 0;
    parser_data->status      = PARSING_COMPLETE;
    LOGD(TAG, LOG_FMT("message complete"));
    return ESP_OK;
}
...
static int read_block(httpd_req_t *req, size_t offset, size_t length)
{
    ...
    if (nbytes < 0) {
        ...
        LOGD(TAG, LOG_FMT("connection closed"));
        /* Connection closed by client so no
         * need to send error response */
        return HTTPD_SOCK_ERR_FAIL;
    }

    LOGD(TAG, LOG_FMT("received HTTP request block size = %d"), nbytes);
    return nbytes;
}
...
static int parse_block(http_parser *parser, size_t offset, size_t length)
{
    ...
    if (!length) {
        ...
        switch (data->status) {
            ...
            case PARSING_HDR_VALUE:
                ...
                break;
            default:
                LOGE(TAG, LOG_FMT("unexpected state"));
    ...
    if (data->status == PARSING_FAILED) {
        ...
        LOGW(TAG, LOG_FMT("parsing failed"));
        return -1;
    } else if (data->paused) {
        ...
        data->status = PARSING_FAILED;
        LOGW(TAG, LOG_FMT("incomplete (%"NEWLIB_NANO_COMPAT_FORMAT"/%"NEWLIB_NANO_COMPAT_FORMAT") with parser error = %d"),
    ...
    LOGD(TAG, LOG_FMT("parsed block size = %"NEWLIB_NANO_COMPAT_FORMAT), NEWLIB_NANO_COMPAT_CAST((offset + nparsed)));
...
static esp_err_t httpd_parse_req(struct httpd_data *hd)
{
    ...
    LOGD(TAG, LOG_FMT("parsing complete"));
    return httpd_uri(hd);
}
...
esp_err_t httpd_req_new(struct httpd_data *hd, struct sock_db *sd)
{
    ...
    /* Handle WebSocket */
    LOGD(TAG, LOG_FMT("New request, has WS? %s, sd->ws_handler valid? %s, sd->ws_close? %s"),
    ...
    if (sd->ws_handshake_done && sd->ws_handler != NULL) {
        ...
        LOGD(TAG, LOG_FMT("New WS request from existing socket, ws_type=%d"), ra->ws_type);
        ...
        if (ra->ws_type == HTTPD_WS_TYPE_CLOSE) {
            ...
            LOGD(TAG, LOG_FMT("Received PONG frame"));
        }
...
esp_err_t httpd_req_get_cookie_val(httpd_req_t *req, const char *cookie_name, char *val, size_t *val_size)
{
    ...
    if (cookie_str == NULL) {
        LOGE(TAG, "Failed to allocate memory for cookie string");
        return ESP_ERR_NO_MEM;
    }

    if (httpd_req_get_hdr_value_str(req, "Cookie", cookie_str, hdr_len_cookie + 1) != ESP_OK) {
        LOGW(TAG, "Cookie not found in header uri:[%s]", req->uri);
        free(cookie_str);
        return ESP_ERR_NOT_FOUND;
    }

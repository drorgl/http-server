...
This example demonstrates the HTTPD server using WebSocket features including **extension negotiation** as per RFC 6455 Section 9.
...
This example demonstrates WebSocket extensions negotiation as per RFC 6455 Section 9. Extensions allow
enhancement of the WebSocket protocol functionality beyond the base specification.

**Extension Configuration:**

To enable extension support in a WebSocket URI handler, set the `supported_extensions` field:
...
static const httpd_uri_t ws_with_extensions = {
        ...
        .user_ctx   = NULL,
        .is_websocket = true,
        .supported_extensions = "permessage-deflate,compress"  // Server supports these extensions
};
```

**How Extensions Work:**

1. **Client Offers Extensions**: During handshake, client sends `Sec-WebSocket-Extensions` header with offered extensions
2. **Server Negotiation**: Server finds intersection between client offers and server-supported extensions
3. **Response**: Server includes negotiated extensions in `Sec-WebSocket-Extensions` response header
4. **Protocol Enhancement**: Negotiated extensions modify WebSocket framing for features like compression

**Example Endpoints:**

This example provides two endpoints:
- `/ws` - Standard WebSocket (no extensions, backward compatible)
- `/ws-ext` - WebSocket with extensions support (`permessage-deflate,compress`)

**Testing Extensions:**

Use a WebSocket client that supports extensions. The server will log which extensions were negotiated
during the handshake process. For example, Chrome/Safari browsers offer `permessage-deflate` extension
while Firefox offers `deflate-stream` (which won't match and will result in no negotiated extensions).
